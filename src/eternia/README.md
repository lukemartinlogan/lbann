...
**It lifts the VRAM ceiling, and by a lot -- but only once the Clio HBM tier is
sized sensibly.** Peak GPU memory, sampled with nvidia-smi across whole runs,
with `HOST_WEIGHTS` + `OWN_WEIGHTS` and a few MiB of page cache:

| fc1 weights | stock (W in VRAM) | paged, 1 GiB HBM tier | paged, 64 MiB HBM tier |
|-------------|-------------------|-----------------------|------------------------|
| 2 MiB       | 158 MiB           | 1238 MiB              | 278 MiB  |
| 64 MiB      | 298 MiB           | 1250 MiB              | 290 MiB  |
| 256 MiB     | 408 MiB           | 1266 MiB              | --       |
| **1 GiB**   | **4298 MiB**      | 1326 MiB              | **366 MiB** |

The middle column is a CONFIGURATION ARTEFACT, not a property of the design,
and it was mistaken for one here. `clio.alt.yaml` reserves a 1024 MB `hbm`
tier -- carried over from the LAMMPS work -- and that reservation is a GPU
allocation taken at startup whatever the model does. It made the paged path
look like it carried a fixed ~1.2 GiB overhead, and led this file to claim
first that paging saved no VRAM at all, and then that it only paid above a
gigabyte of weights. Both readings came from the same 1 GiB of config.

Sizing the tier to 64 MB instead:

- the overhead largely disappears -- 1238 MiB -> 278 MiB on the small model;
- the crossover drops from ~1 GiB of weights to somewhere near 64 MiB, where
  paged (290 MiB) and stock (298 MiB) are already level;
- at 1 GiB of weights the saving is **4298 MiB -> 366 MiB, 11.7x**, with the
  objective identical to stock (1.92597 / 5.80251) and `get_errors=0`.

The tier still has to exist -- paged blobs prefer it, and it is what keeps hot
pages off the host -- so this is a sizing decision, not a removal. Its right
size is a few multiples of the page cache, not a fixed fraction of the GPU.
